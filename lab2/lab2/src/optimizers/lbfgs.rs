...
impl Lbfgs {
    ...
    pub fn minimize(&self, f: &impl Objective<f64>, start: &[f64]) -> OptimizeResult<f64> {
        ...
        let mut path_iters: Vec<usize> = if stride > 1 {
            vec![0usize]
        } else {
            Vec::new()
        };
        ...
        for it in 0..self.max_iters {
            ...
            if g_norm < self.tol {
                if stride > 1 {
                    if path_iters.last().copied() != Some(it) {
                        path.push(x.clone());
                        path_iters.push(it);
                    }
                }
                return OptimizeResult {
                    ...
                    grad_calls,
                    path,
                    path_iters: if stride > 1 { path_iters } else { Vec::new() },
                };
            }
            ...
            if stride <= 1 {
                path.push(x.clone());
            } else if completed % stride == 0 || completed == self.max_iters {
                path.push(x.clone());
                path_iters.push(completed);
            }
        }

        if stride > 1 && path_iters.last().copied() != Some(self.max_iters) {
            path.push(x.clone());
            path_iters.push(self.max_iters);
        }
