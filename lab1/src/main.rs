...
fn verdict_constructive(result: &OptimizeResult<ConstructiveNumber>, max_iters: usize) -> String {
    let value_width = result.best_value.width();

    if value_width < 1e-4 && result.iterations < max_iters {
        "narrow interval, strong result".to_string()
    } else if value_width < 1.0 {
        "usable interval estimate".to_string()
    } else if result.iterations >= max_iters {
        "iteration limit reached, interval still wide".to_string()
    } else {
        "interval remains too wide".to_string()
    }
}
